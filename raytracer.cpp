...
float RDM_G1(float DdotH, float DdotN, float alpha) {
  float b, k;
  float cos2TetaX = DdotN * DdotN;
  float tanTetaX = sqrt(1.f - cos2TetaX) / DdotN;
  ...
  return RDM_chiplus(DdotH / DdotN);
}
...
color3 RDM_bsdf_s(float LdotH, float NdotH, float VdotH, float LdotN, float VdotN, Material *m) {
  ...
  // using D = RDB_Beckmann, F = RDM_Fresnel, G = RDM_Smith
...
color3 RDM_bsdf_d(Material *m) {
  return color3(m->diffuseColor / 3.14159f); 
}
